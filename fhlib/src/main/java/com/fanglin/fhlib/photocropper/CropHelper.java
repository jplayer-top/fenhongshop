...
public class CropHelper {
    ...
    public static Uri generateUri() {
        ...
        String name = String.format(Locale.CHINA, "image-%d.jpg", System.currentTimeMillis());
    ...
    private static Intent buildCropIntent(String action, CropParams params) {
        ...
        i.setDataAndType(params.uri, params.type);
        i.putExtra(MediaStore.EXTRA_OUTPUT, params.uri);

        //i.putExtra("crop", params.crop);//需要注释掉 否则会出现从相册添加后无法裁剪

        i.putExtra("scale", params.scale);

        i.putExtra("aspectX", params.aspectX);
        i.putExtra("aspectY", params.aspectY);
        i.putExtra("outputX", params.outputX);
        i.putExtra("outputY", params.outputY);

        i.putExtra("return-data", params.returnData);
        i.putExtra("outputFormat", params.outputFormat);
        i.putExtra("noFaceDetection", params.noFaceDetection);
        i.putExtra("scaleUpIfNeeded", params.scaleUpIfNeeded);
        return i;
